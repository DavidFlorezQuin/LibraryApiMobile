...
class UserFormulario : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?){
        ...
        btnGuardar.setOnClickListener{
            ...
            val tipoUsiario = txtTipoUsuario.selectedItemId.toInt() +1

            if (user != null) {
                val newUser = User(user!!.id, nombre, direccion, correo, tipoUsiario)
                actualizarUsuario(newUser)
            } else {
                val newUser = User(0, nombre, direccion, correo, tipoUsiario)
